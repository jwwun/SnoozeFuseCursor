...
"SnoozeFuse: The innovative nap timer that knows when you've fallen asleep! Hold & release to automatically start your nap countdown. Perfect for power naps with customizable timers, no more oversleeping or setting complicated alarms. Sleep smarter, not harder!"
...
- **Default Notification Permission Warning**: Changed the default behavior to always show the notification permission warning on the main settings screen until explicitly dismissed by the user. This ensures users are aware of the importance of enabling notifications for the app to function properly when in the background.
...
- Notification warning UI is shown in main Settings by default until dismissed by user
...
1. Notification permission warning is shown on the main Settings screen by default
2. Users can hide the warning using the "Hide This" button if they don't want to enable notifications
3. Critical alerts are prominently featured at the top of Advanced Settings when notifications are enabled
4. The Advanced Settings screen provides detailed control of notification features
5. Critical alerts are highlighted as an essential feature for reliable alarm functionality
6. Regular notifications serve as a backup when the app is completely terminated
7. When notifications are enabled, the app checks if critical alerts are also authorized
